+(function(M, T, p) {
	...
	function Xc() {
		...
		this.$get = ["$injector", "$exceptionHandler", "$parse", function(a, c, d) {
			function e() {
				this.$id = Fa();
				this.$$phase = this.$parent = this.$$watchers = this.$$nextSibling = this.$$prevSibling = this.$$childHead = this.$$childTail = null; + this["this"] = this.$root = this;
				this.$$destroyed = !1;
				this.$$asyncQueue = [];
				this.$$listeners = {};
			...
			e.prototype = {
				...
				$on: function(a, b) {
					var c = this.$$listeners[a]; + c || (this.$$listeners[a] = c = []);
					c.push(b);
					return function() {
						c[Ga(c, b)] = null
					}
	...
	n({
		...
		text: t(Z < 9 ? function(b, a) {
			if (b.nodeType == 1) {
				...
				if (C(a)) return b.nodeValue; + b.nodeValue = a
			}
		} : function(b, a) {
	...
	var Wa = function(a) {
		...
		xd = ["$scope", "$exceptionHandler", "$attrs", "$element", "$parse", function(a, c, d, e, g) {
			function i(a, c) {
				c = c ? "-" + bb(c, "-") : ""; + e.removeClass((a ? Ra : Sa) + c).addClass((a ? Sa : Ra) + c)
			}
	...
	(function(a) {
		...
		Aa("ng", ["ngLocale"], ["$provide", function(a) {
			a.provider("$compile", Jb).directive({
				...
				ngRequired: jc,
				ngValue: Cd
			}).directive(pb).directive(kc); + a.provider({
				$anchorScroll: Cc,
				$animation: Ib,
				$animator: qd,
				$browser: Ec,
